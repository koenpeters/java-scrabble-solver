package org.solvr.scrabble.util;
 ...
 */
public class TimingSingleton {

	private Logger log = Logger.getLogger(TimingSingleton.class);
	...
	*/
	...
	private class TimingGroup {
		...
		void stop(int id) {
			...
			if (running[id] == 0) {
				log.error("called stop before calling start with id " + id);
			} else {
